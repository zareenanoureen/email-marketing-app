...
# Helper function to summarize stacks in chunks
...
def format_brand_summary(summary):
    # Define the structure of the expected summary
    lines = summary.strip().split('\n')
    
    # Define the keys you expect in the summary
    keys = ['Name', 'Address', 'Phone Number', 'Email', 'Unique Selling Points']
    ...
    for line in lines:
        for key in keys:
            if line.startswith(f"{key}:"):
                # Extract value and strip leading/trailing spaces
                value = line[len(key) + 1:].strip()
                result[key] = value
    return result


def extract_technology_stacks(summary):
    ...
    for line in lines:
        ...
        # Check if the line has at least 2 characters and starts with a number followed by a period
        if len(line) > 2 and line[0].isdigit() and line[1] == '.':
            # Extract the number and the description
            try:
                number = line.split('.')[0].strip()
                description = line.split('.', 1)[1].strip()
                
                # Add to the result list as a dictionary
